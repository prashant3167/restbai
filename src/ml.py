...
def get_advert_title(features):
    ...
    response = openai.Completion.create(
        ...
        prompt="Give a catchy one liner advertisement headline for property also mention following features: " + str(features),
        temperature=0,
        max_tokens=64,
        top_p=1.0,
...
def get_advert_desc(features):
    ...
    response = openai.Completion.create(
        model="text-davinci-003",
        prompt="Give a catchy 5 liner advertisement description for property also mention following features: " + str(
            features),
        temperature=0,
        max_tokens=64,
        top_p=1.0,
...
def get_mentioned_feature(data):
    ...
    return interior + kitchen + appliances + \
        [str(data['totalBedroom']) + "bedrooms"] + [str(data['totalWashroom']) + "washrooms"]